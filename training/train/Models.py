...
MODEL_CONFIGS = {
	...
	# MLDecoder head
	'SWModel27': {'class': 'ViT', 'num_blocks': 12, 'patch_size': 16, 'd_model': 768, 'mlp_dim': 768*4, 'num_heads': 12, 'stochdepth_rate': 0.05, 'use_sine': True, 'cnn_stem': 'conv:c=64;ln;relu;conv:c=128;ln;relu;conv:c=256;ln;relu;conv:c=512;ln;relu;conv:c=768,s=1,k=1,p=0', 'ml_decoder_head': True},
}
...
class VisionModel(nn.Module):
	...
	def load_model(path: Path | str, device: str | None = None) -> 'VisionModel':
		...
		model = VisionModel.from_config(config)
		model.load(resume)
...
def asl_helper(preds: torch.Tensor, target: torch.Tensor):
...
def basic_calculate_loss(preds: dict[str, torch.Tensor], batch: dict[str, torch.Tensor], loss_type: str):
	if loss_type == "ce":
		...
		loss = F.binary_cross_entropy_with_logits(preds['tags'], batch['tags']) * 128
	elif loss_type == "asl6":
		loss = F.binary_cross_entropy_with_logits(preds['tags'], batch['tags']) * 256
	elif loss_type == "asl7":
		loss = F.binary_cross_entropy_with_logits(preds['tags'], batch['tags']) * 2
	elif loss_type == 'asl8':
		loss = (asymmetric_loss(preds['tags'], batch['tags'], gamma_pos=1., gamma_neg=4., clip=0.05) * 256).mean()
	elif loss_type == 'asl9':
		loss = (asymmetric_loss(preds['tags'], batch['tags'], gamma_pos=1., gamma_neg=4., clip=0.05) * 358).mean()
	else:
...
def asymmetric_loss(x: torch.Tensor, y: torch.Tensor, gamma_pos: float, gamma_neg: float, clip: float | None, eps: float = 1e-8, disable_torch_grad_focal_loss: bool = True) -> torch.Tensor:
	"""
	Copyright (c) 2020 Alibaba-MIIL
	MIT License
	https://github.com/Alibaba-MIIL/ASL/blob/8c9e0bd8d5d450cf19093363fc08aa7244ad4408/src/loss_functions/losses.py

	Implementation of Asymmetric Loss (ASL) from "Asymmetric Loss for Multi-Label Classification"
	https://arxiv.org/abs/2009.14119

	:param x: Tensor of shape (batch_size, num_classes) with logits
	:param y: Tensor of shape (batch_size, num_classes) with labels
	:param gamma_pos: Positive asymmetry parameter
	:param gamma_neg: Negative asymmetry parameter
	:param clip: Clip value for negative predictions
	:param eps: Epsilon value for numerical stability
	:param disable_torch_grad_focal_loss: Whether to disable torch grad for focal loss
	...
	# Cross Entropy
	loss_pos = y * torch.log(xs_pos.clamp(min=eps))
	loss_neg = (1 - y) * torch.log(xs_neg.clamp(min=eps))
...
def sinusoidal_position_embedding(width: int, height: int, depth: int, dtype: torch.dtype, device: torch.device, temperature: float = 10000):
	"""
...
class CLIPLikeModel(VisionModel):
	...
	def calculate_loss(self, preds, batch):
		return basic_calculate_loss(preds, batch, self.loss_type)
...
class MaskedAutoEncoderViT(nn.Module):
	...
	def calculate_loss(self, preds, batch):
		return basic_calculate_loss(preds, batch, self.loss_type) + preds['mae_loss']
...
class ViT(VisionModel):
	def __init__(self,
		...
		if self.ml_decoder_head:
			self.head = MLDecoder(num_classes=n_tags, d_model=d_model, num_groups=200, initial_num_features=d_model)
		else:
			self.head = nn.Linear(d_model, out_dim)

	def forward(self, batch: Dict[str, torch.Tensor], return_embeddings: bool = False, return_loss: bool = False):
		with torch.amp.autocast_mode.autocast('cuda', enabled=self.use_amp):
			...
			if return_loss:
				result['loss'] = self.calculate_loss(result, batch)

			return result
	
	def calculate_loss(self, preds: dict[str, torch.Tensor], batch: dict[str, torch.Tensor]):
		return basic_calculate_loss(preds, batch, self.loss_type)
...
class MLDecoder(nn.Module):
	def __init__(self, num_classes: int, d_model: int, num_groups: int = 100, initial_num_features: int = 2048):
		super().__init__()
		self.num_classes = num_classes
		self.num_groups = min(num_groups, num_classes)

		self.embed_standart = nn.Linear(initial_num_features, d_model)

		query_embed = nn.Embedding(self.num_groups, d_model)
		query_embed.requires_grad_(False)

		decoder_dropout = 0.1
		dim_feedforward = 2048
		layer_decode = TransformerDecoderLayerOptimal(d_model=d_model, dim_feedforward=dim_feedforward, dropout=decoder_dropout)
		self.decoder = nn.TransformerDecoder(layer_decode, num_layers=1)
		#self.decoder.embed_standart = self.embed_standart
		self.decoder.query_embed = query_embed

		self.decoder.num_classes = num_classes
		self.decoder.duplicate_factor = int(num_classes / self.num_groups + 0.999)
		self.decoder.duplicate_pooling = nn.Parameter(torch.Tensor(self.num_groups, d_model, self.decoder.duplicate_factor))
		self.decoder.duplicate_pooling_bias = nn.Parameter(torch.Tensor(self.num_classes))
	
		nn.init.xavier_normal_(self.decoder.duplicate_pooling)
		nn.init.zeros_(self.decoder.duplicate_pooling_bias)
		self.decoder.group_fc = GroupFC(self.num_groups)
	
	def forward(self, x: torch.Tensor) -> torch.Tensor:
		embedding_spatial = x
		embedding_spatial_768 = self.embed_standart(embedding_spatial)
		embedding_spatial_768 = F.relu(embedding_spatial_768, inplace=True)

		bs = embedding_spatial_768.shape[0]
		query_embed = self.decoder.query_embed.weight
		tgt = query_embed.unsqueeze(1).expand(-1, bs, -1)
		h = self.decoder(tgt, embedding_spatial_768.transpose(0, 1))
		h = h.transpose(0, 1)
		...
		h_out = h_out + self.decoder.duplicate_pooling_bias
		logits = h_out
